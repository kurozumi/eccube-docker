...
- **`app/template/` に本体のテンプレートを丸ごと写さない。** 写した瞬間は同じでも、
  ...
  違わなかった）ので消した。
- **プラグインがテーマへ写すのは、いまはメールの文面だけ**（`.gitignore` 済み）。
  画面のテンプレートは `dtb_page.file_name` がプラグインの中を指すようになり、
  写さなくなった（2026-09、10プラグイン・25ページ）。**テーマを変えても
  写し直しが要らない。** 店がテンプレートを直すのは**プラグインテンプレート
  編集プラグイン**の役目で、直した内容はデータベースに残る。
  写しは無効化→有効化で作り直せる。**店が直したものだけ `git add -f` で追跡する。**
