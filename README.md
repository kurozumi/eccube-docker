...
```
.
├── compose.yaml            # base（ec-cube / nginx / db / redis / redis-session）
...
│   └── config/eccube/packages/   # logging / cache / trusted_proxies
...
│   └── caddy/Caddyfile
└── bin/{init,switch-version,reset,publish,healthcheck,assets,test}.sh
```
