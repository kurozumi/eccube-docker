...
## 🏢 B2B（法人向け）EC サイトもこの環境で作れる

> **[EC-CUBE で B2B サイト](https://b2b.eccube-plugin.net/)**  
> https://b2b.eccube-plugin.net/

電話や FAX で受けている法人取引を Web に載せるための **EC-CUBE 4.4 用プラグイン群（28 本）**
を紹介しているサイト。本体は無料のオープンソース、プラグインは**買い切り（月額なし）**で、
必要なものだけ組み合わせる。

- 会員グループ別の卸価格出し分け、会員限定サイト、会員登録の承認制
- 掛売り・月締め請求、適格請求書対応、入金消し込み
- 取引先ごとの複数担当者管理、発注承認フロー、型番一括発注、見積依頼
- 商品スペック検索、製品比較

プラグインは `app/Plugin/` に置いて `bin/plugin.sh add` で導入できる。この環境で
立ち上げた EC-CUBE にそのまま載せれば、**B2C の店舗と同じ手順で B2B サイトを公開できる。**
...
- Mac / Linux のターミナル。**Windows は WSL2（Ubuntu）の中で**使う（`bin/` は bash。PowerShell では動かない）
- DB は MariaDB / MySQL（既定）か PostgreSQL（`.env` の `DB_ENGINE`、版は `MARIADB_VERSION` / `PG_VERSION`）。どちらも同梱、外部 DB も可
- PHP は配布イメージのタグで選ぶ（4.2: 8.1/8.2、4.3: 8.1〜8.3、4.4: 8.2〜8.5）。**毎週焼き直され、パッチが入る**
- amd64 / arm64 どちらも可
