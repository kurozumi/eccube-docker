...
| --- | --- | --- | --- | --- |
| **受注・会員・商品**（DB） | ボリューム `db_data`（PostgreSQL は `pg_data`） | 残る | **消える** | **消える** |
...
`down -v` を打たなくても、**別のサーバーへ移すときに落ちるもの**がある。
管理画面がディスクに書いたファイル、コンテナ内の `.env`、そして**ホストの `.env`**。
DB と画像だけ持っていくと、`dtb_page` には行があるのに twig が無くて、
そのページだけ 500 になる。**`.env` の `ECCUBE_AUTH_MAGIC` を持っていかないと、
DB を戻しても誰もログインできない**（全パスワードのハッシュの鍵）。
一覧と対処は [バックアップ / 復元](backup.md) の「原則」。
