...
- **`.env` は平文で、そのサーバーの中にだけある。** 十分なことが多いが、チームで扱う・
  複数台に配る・監査が要るなら、`.env` を外から作る形にできる（#101）。この環境は
  `.env` を「起動時に読む 1 ファイル」としか見ていないので、どの方法でも動く:
  - **sops + age**: 暗号化した `.env.enc` を git に入れ、起動前に
    `sops -d .env.enc > .env && chmod 600 .env`（鍵はサーバーの `~/.config/sops/age/keys.txt`）
  - **1Password / Bitwarden CLI**: `op run --env-file .env.tpl -- docker compose up -d`
    のように、値を書き込まず環境変数として渡す（compose は環境変数を `.env` より優先する）
  - **クラウドの Secrets Manager**: 起動スクリプトで取り出して `.env` を生成
  compose の `secrets:` は使っていない。EC-CUBE 本体が `DATABASE_URL` 等を環境変数から
  読む作りで、ファイル経由の秘密（`_FILE` 方式）に対応していないため
...
`ECCUBE_IMAGE` があれば pull、なければビルドする。シークレットの生成、空いているポートの選択
（8080 が使われていれば別の番号）、DB の作成、初期データの投入まで通しで行い、**終わるまで待って
URL とログイン情報を表示する**。Docker が無い・動いていない・Compose が古いときは、その旨と
やることを表示して止まる。
